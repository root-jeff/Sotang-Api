...
import {
  LoginBody, RegisterBody, RefreshBody, LogoutBody,
  AuthTokensResponse, MessageResponse, ErrorResponse,
  type RegisterBodyType, type LoginBodyType, type RefreshBodyType,
} from './auth.schema';
...
export default async function authRoutes(app: FastifyInstance) {
  ...
  // ── POST /auth/register ───────────────────────────────────────────────────
  app.post<{ Body: RegisterBodyType }>('/register', {
...
**Rate limit:** 5 peticiones/minuto por IP.
      ...
      body: RegisterBody,
      response: {
        201: { ...AuthTokensResponse, description: 'Usuario creado. Tokens listos para usar.' },
        409: { ...ErrorResponse, description: 'El email ya está registrado.' },
        422: { ...ErrorResponse, description: 'Validación fallida (email inválido, password muy corta, etc.)' },
        429: { ...ErrorResponse, description: 'Rate limit: máximo 5 registros/minuto por IP.' },
      },
    },
  }, async (req, reply) => {
...
**Rate limit:** 10 peticiones/minuto por IP.
      ...
      body: LoginBody,
      response: {
        200: { ...AuthTokensResponse, description: 'Login exitoso. Guarda los tokens de forma segura (SecureStore en mobile).' },
        401: { ...ErrorResponse, description: 'Credenciales inválidas (email o contraseña incorrectos).' },
        429: { ...ErrorResponse, description: 'Rate limit: máximo 10 intentos/minuto por IP.' },
      },
    },
  }, async (req, reply) => {
...
4. Reintenta el request original
      ...
      body: RefreshBody,
      response: {
        200: { ...AuthTokensResponse, description: 'Tokens renovados. El refresh token anterior ya no es válido.' },
        401: { ...ErrorResponse, description: 'Refresh token inválido, expirado o ya revocado.' },
      },
    },
  }, async (req, reply) => {
...
**Requiere:** \`Authorization: Bearer {accessToken}\`
      ...
      body: LogoutBody,
      response: {
        200: { ...MessageResponse, description: 'Sesión cerrada. Refresh token revocado.' },
