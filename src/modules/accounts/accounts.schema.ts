...
export const CreateAccountBody = Type.Object(
  {
    nombre: Type.String({
      minLength: 1,
      maxLength: 100,
      example: "Banco Guayaquil",
    }),
    tipo: AccountType,
    moneda: Type.Optional(
      Type.String({
        minLength: 3,
        maxLength: 3,
        default: "USD",
    ...
    saldoInicial: Type.Optional(
      Type.Number({ minimum: 0, default: 0, example: 1200.5 }),
    ),
    color: Type.Optional(
      Type.String({
        pattern: "^#[0-9A-Fa-f]{6}$",
    ...
    tarjeta: Type.Optional(
      Type.Object({
        ...
        ultimos4: Type.Optional(
          Type.String({ minLength: 4, maxLength: 4, example: "1234" }),
        ),
        tasaInteresAnual: Type.Optional(
          Type.Number({ minimum: 0, example: 16.77 }),
        ),
        cupoGrupoId: Type.Optional(
          Type.String({
            format: "uuid",
            description:
              "ID del grupo de cupo compartido (ej: Diners + Titanium)",
          }),
        ),
      }),
    ),
    // Solo para tipo cripto
    cripto: Type.Optional(
      Type.Object({
    ...
    ahorroVirtual: Type.Optional(
      Type.Object({
        cuentaPadreId: Type.String({
          format: "uuid",
          description: "ID de la cuenta real que contiene este ahorro virtual",
        }),
      }),
    ),
  },
