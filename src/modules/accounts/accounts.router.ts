...
- **ahorro_virtual** → campo \`ahorroVirtual\` (obligatorio, referencia a cuenta real existente)`,
    ...
    try {
      ...
      if (err.message === 'PARENT_NOT_FOUND') {
        return reply.status(404).send({ statusCode: 404, error: 'Not Found', message: 'Cuenta padre no encontrada o no pertenece al usuario' });
      }
      throw err;
    }
  });
  ...
  app.get('/', {
    ...
    schema: {
      ...
      description: 'Retorna todas las cuentas activas del usuario autenticado, ordenadas por `orden` y luego alfabéticamente.',
      security: [{ bearerAuth: [] }],
      response: {
        200: Type.Object({
  ...
  app.get('/summary', {
    ...
    schema: {
      ...
      description: 'Saldo total en USD (cuentas con `incluirEnTotal=true`) y desglose por tipo de cuenta.',
      security: [{ bearerAuth: [] }],
      response: {
        200: Type.Object({
  ...
  app.get('/:id', {
    ...
    if (!account) return reply.status(404).send({ statusCode: 404, error: 'Not Found', message: 'Cuenta no encontrada' });
    return account;
  });
  ...
  app.patch('/:id', {
    ...
    schema: {
      ...
      description: 'Permite modificar nombre, color, icono, notas, orden e `incluirEnTotal`. El tipo y moneda no son modificables.',
      security: [{ bearerAuth: [] }],
      params: Type.Object({ id: Type.String({ format: 'uuid' }) }),
    ...
    if (!updated) return reply.status(404).send({ statusCode: 404, error: 'Not Found', message: 'Cuenta no encontrada' });
    return updated;
  });
  ...
  app.delete('/:id', {
    ...
    schema: {
      ...
      description: 'Soft-delete: marca la cuenta como inactiva (`activa = false`). Los datos históricos se conservan.',
      security: [{ bearerAuth: [] }],
      params: Type.Object({ id: Type.String({ format: 'uuid' }) }),
      response: {
    ...
    if (!deleted) return reply.status(404).send({ statusCode: 404, error: 'Not Found', message: 'Cuenta no encontrada' });
