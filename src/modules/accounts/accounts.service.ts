...
import {
  accounts, cardConfigs, cryptoConfigs, savingsVirtualConfigs,
...
export class AccountsService {
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (data.tipo === 'tarjeta_credito' && data.tarjeta) {
        await tx.insert(cardConfigs).values({
          ...
          tasaInteresAnual: data.tarjeta.tasaInteresAnual != null
      ...
      if (data.tipo === 'cripto' && data.cripto) {
        await tx.insert(cryptoConfigs).values({
          ...
          precioCompraPromedio: data.cripto.precioCompraPromedio != null
  ...
  async updateAccount(usuarioId: string, id: string, data: Partial<CreateAccountBodyType>) {
    const updateData: Record<string, unknown> = {};
    if (data.nombre         !== undefined) updateData.nombre         = data.nombre;
    if (data.color          !== undefined) updateData.color          = data.color;
    if (data.icono          !== undefined) updateData.icono          = data.icono;
    if (data.incluirEnTotal !== undefined) updateData.incluirEnTotal = data.incluirEnTotal;
    if (data.orden          !== undefined) updateData.orden          = data.orden;
    if (data.notas          !== undefined) updateData.notas          = data.notas;

    if (Object.keys(updateData).length === 0) return this.getAccount(usuarioId, id);
